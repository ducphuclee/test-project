...
### Bước 1 — Load design knowledge

Kiểm tra xem project có CSV database chưa:

```bash
ls .project-info/design-data/ 2>/dev/null
```

**Nếu có** (bootstrapped với Phase 3.5.1) → dùng search script thay vì đọc text:

```bash
# Tìm color tokens liên quan
node scripts/design-search.js "primary button color" --csv colors

# Tìm component phù hợp
node scripts/design-search.js "form input field" --csv components

# Kiểm tra UX rules trước khi code
node scripts/design-search.js "prefix tailwind" --csv ux-guidelines

# Search toàn bộ (tất cả 4 CSVs)
node scripts/design-search.js "error state destructive"
```

Mỗi query trả về top 3 kết quả phù hợp nhất theo BM25 scoring.

**Nếu không có** → đọc `.project-info/conventions/design.md` thủ công.

### Bước 2 — Apply

- Match visual system đã có — không tự introduce tokens mới
- Dùng đúng class prefix của project (ví dụ: `tw-` nếu rules yêu cầu)
...
> Bootstrap sẽ tạo script này tự động. Nếu thiếu, tạo tại `scripts/design-search.js`.

Script đọc CSV từ `.project-info/design-data/` và chạy BM25 search:

```
Usage: node scripts/design-search.js "<query>" [options]

Options:
  --csv <name>   colors | typography | components | ux-guidelines
  --top <n>      Số kết quả (default: 3)

Examples:
  node scripts/design-search.js "primary button"
  node scripts/design-search.js "heading font" --csv typography
  node scripts/design-search.js "modal dialog" --csv components --top 5
```
